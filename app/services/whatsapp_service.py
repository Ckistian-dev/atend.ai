...
class WhatsAppService:
    def __init__(self):
        ...
        self.evo_api_url = settings.EVOLUTION_API_URL
        self.evo_api_key = settings.EVOLUTION_API_KEY
        self.evo_headers = {"apikey": self.evo_api_key, "Content-Type": "application/json"}
        ...
        try:
            # --- ALTERADO: Usar postgresql+asyncpg explicitamente se for postgres ---
            ...
            if evo_db_url: # Apenas cria engine se a URL estiver definida
                 self.evolution_db_engine = create_async_engine(
                    evo_db_url,
                    pool_size=5, max_overflow=10, pool_timeout=30, pool_recycle=1800 # Adicionado recycle
                )
    ...
    def _normalize_number(self, number: str) -> str:
        """Limpa número (remove não dígitos)."""
        clean_number = "".join(filter(str.isdigit, str(number)))
        return clean_number

    async def get_connection_status_evolution(self, instance_name: str) -> dict:
        """Verifica status da conexão Evolution."""
        if not instance_name:
            return {"status": "no_instance_name"}
        url = f"{self.evo_api_url}/instance/connectionState/{instance_name}"
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(url, headers={"apikey": self.evo_api_key})
            ...
            # Retorna texto de erro da API se disponível
            error_detail = e.response.text if e.response else str(e)
            return {"status": "disconnected"} if e.response.status_code == 404 else {"status": "api_error", "detail": error_detail}
        except Exception as e:
    ...
    async def _get_qrcode_and_instance_data_evolution(self, instance_name: str) -> Optional[Dict[str, Any]]:
        """Busca QR Code da Evolution."""
        url = f"{self.evo_api_url}/instance/connect/{instance_name}"
        async with httpx.AsyncClient(timeout=90.0) as client:
            response = await client.get(url, headers={"apikey": self.evo_api_key})
            response.raise_for_status() # Lança erro se não for 2xx
            data = response.json()
            qr_code_string = data.get('base64') or data.get('qrcode', {}).get('base64') or data.get('code')
            if not qr_code_string:
                logger.error(f"Evolution: API não retornou um QR Code válido para '{instance_name}'. Resposta: {data}")
                raise Exception("API Evolution não retornou um QR Code válido.")

            instance_data = data.get("instance", {})
            instance_data['qrcode'] = qr_code_string
            logger.info(f"Evolution: QR Code obtido para '{instance_name}'.")
            return instance_data

    async def _create_instance_evolution(self, instance_name: str):
        """Cria instância na Evolution."""
        # Garante que a URL do webhook termine com /
        webhook_url = settings.WEBHOOK_URL
        if webhook_url:
             webhook_url = f"{webhook_url.rstrip('/')}/evolution/messages-upsert"
        else:
        ...
        payload = {
            ...
            "qrcode": True,
            # Só adiciona webhook se a URL estiver definida
            **({"webhook": {
                "url": webhook_url,
                "enabled": True,
        ...
        url = f"{self.evo_api_url}/instance/create"
        async with httpx.AsyncClient(timeout=60.0) as client:
            response = await client.post(url, headers=self.evo_headers, json=payload)
            response.raise_for_status()
            logger.info(f"Instância Evolution '{instance_name}' criada.")
            return response.json()

    async def create_and_connect_instance_evolution(self, instance_name: str) -> dict:
        """Conecta ou cria e conecta instância Evolution."""
        try:
            instance_data = await self._get_qrcode_and_instance_data_evolution(instance_name)
            return {"status": "qrcode", "instance": instance_data}
        except httpx.HTTPStatusError as e:
            if e.response.status_code != 404:
                error_detail = e.response.text
                logger.error(f"Evolution: Erro HTTP {e.response.status_code} ao conectar '{instance_name}': {error_detail}")
        ...
        try:
            creation_data = await self._create_instance_evolution(instance_name)
            qr_code_string = creation_data.get('hash', {}).get('qrcode')
            instance_info = creation_data.get('instance', {})
            if qr_code_string and instance_info:
                  instance_info['qrcode'] = qr_code_string
                  logger.info(f"Evolution: Instância '{instance_name}' criada e QR Code obtido.")
                  return {"status": "qrcode", "instance": instance_info}
            else:
                  logger.error(f"Evolution: Falha ao obter QR Code após criar '{instance_name}'. Resposta: {creation_data}")
                  # Tenta buscar o QR Code separadamente como fallback
                  await asyncio.sleep(2) # Pequena pausa antes de tentar buscar de novo
                  instance_data_fallback = await self._get_qrcode_and_instance_data_evolution(instance_name)
                  return {"status": "qrcode", "instance": instance_data_fallback}

        except Exception as e:
            error_detail = e.response.text if hasattr(e, 'response') and hasattr(e.response, 'text') else str(e)
            logger.error(f"Evolution: Erro crítico ao CRIAR instância '{instance_name}': {error_detail}", exc_info=True)
            return {"status": "error", "detail": f"Erro ao criar instância: {error_detail}"}


    async def disconnect_instance_evolution(self, instance_name: str) -> dict:
        """Desconecta e remove instância Evolution."""
        url_logout = f"{self.evo_api_url}/instance/logout/{instance_name}"
        url_delete = f"{self.evo_api_url}/instance/delete/{instance_name}"
        try:
            async with httpx.AsyncClient() as client:
                try:
                    logout_resp = await client.delete(url_logout, headers={"apikey": self.evo_api_key}, timeout=30.0)
                    logout_resp.raise_for_status()
                    logger.info(f"Evolution: Logout da instância '{instance_name}' bem-sucedido.")
                except httpx.HTTPStatusError as logout_err:
                      if logout_err.response.status_code not in [404, 500]:
                           logger.warning(f"Evolution: Erro {logout_err.response.status_code} no logout de '{instance_name}', mas prosseguindo com delete: {logout_err.response.text}")

                delete_resp = await client.delete(url_delete, headers={"apikey": self.evo_api_key}, timeout=30.0)
            ...
            return {"status": "error", "detail": f"Erro ao desconectar/remover: {error_detail}"}
    ...
    async def send_text_message_evolution(self, instance_name: str, number: str, text: str) -> Dict[str, Any]:
        ...
        url = f"{self.evo_api_url}/message/sendText/{instance_name}"
        # --- CORREÇÃO: Payload ajustado para o erro "instance requires property 'text'" ---
        payload = {
            "number": jid,
            "text": text, # Envia 'text' diretamente no payload principal
            "options": { "delay": 1200, "presence": "composing", "linkPreview": False }
            # "textMessage": { "text": text } # Remove a chave aninhada
        }
        # --- FIM CORREÇÃO ---
        ...
        for attempt in range(max_retries):
            try:
                async with httpx.AsyncClient() as client:
                    response = await client.post(url, headers=self.evo_headers, json=payload, timeout=30.0)
                    ...
                    # O timestamp pode vir como string ou int/float dependendo da versão
    ...
    def _run_ffmpeg_sync(self, ogg_path: str, mp3_path: str):
        ...
        try:
            result = subprocess.run(command, capture_output=True, check=True, text=True, encoding='utf-8') # Adicionado encoding
    ...
    async def get_media_and_convert_evolution(self, instance_name: str, message: dict) -> Optional[dict]:
        ...
        url = f"{self.evo_api_url}/chat/getBase64FromMediaMessage/{instance_name}"
        payload = {"message": message}
        try:
            async with httpx.AsyncClient() as client:
                response = await client.post(url, json=payload, headers=self.evo_headers, timeout=60)
                response.raise_for_status()
            ...
            if "imageMessage" in message_content:
                ...
                try:
                    with tempfile.TemporaryDirectory() as temp_dir:
                        input_filename = f"{uuid.uuid4()}.oga"
    ...
    async def fetch_chat_history_evolution(self, instance_id: str, number: str, count: int = 32) -> List[Dict[str, Any]]:
        """Busca histórico do banco de dados da Evolution."""
        ...
        if len(normalized_number) == 12 and normalized_number.startswith("55"):
            if len(normalized_number) == 12 and normalized_number[4] != '9':
                 jid_alternativo = f"{normalized_number[:4]}9{normalized_number[4:]}@s.whatsapp.net"
            elif len(normalized_number) == 13 and normalized_number[4] == '9':
                 jid_alternativo = f"{normalized_number[:4]}{normalized_number[5:]}@s.whatsapp.net"
    ...
    async def send_text_message_official(self, phone_number_id: str, access_token: str, to_number: str, text: str) -> Dict[str, Any]:
        ...
        clean_to_number = "".join(filter(str.isdigit, str(to_number)))
        ...
        for attempt in range(max_retries):
            try:
                ...
                if isinstance(e, httpx.HTTPStatusError):
                      ...
                      error_detail = e.response.text if e.response else str(e) # Pega texto da resposta
