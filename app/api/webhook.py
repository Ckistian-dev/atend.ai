...
async def process_incoming_message(data: dict):
    """
    Processa uma mensagem recebida. Se for a primeira, busca todo o histórico
    (incluindo áudios) e transcreve-o antes de processar a mensagem atual.
    """
    async with SessionLocal() as db:
        try:
            ...
            if was_created:
                logger.info(f"Atendimento novo. A buscar histórico de conversas para {contact_number}...")
                raw_history = await whatsapp_service.fetch_chat_history(instance_name, contact_number)
                
                for raw_msg in reversed(raw_history):
                    try:
                        # --- A CORREÇÃO ESTÁ AQUI ---
                        # Garante que a mensagem é um dicionário antes de a usar
                        ...
                        if msg_content.get("conversation") or msg_content.get("extendedTextMessage"):
                            ...
                            logger.info(f"Áudio encontrado no histórico de {contact_number}. A processar...")
                        ...
                        logger.warning(f"Não foi possível processar uma mensagem do histórico: {e} - Mensagem: {raw_msg}")
                        continue
                
                logger.info(f"Histórico de {len(history_list)} mensagens processado.")
