    ...
    async with SessionLocal() as db:
        ...
        for at in items:
            ...
            try:
                ...
                components_to_send = copy.deepcopy(components) if components else []
                if components_to_send:
                    for comp in components_to_send:
                        if 'parameters' in comp:
                            for param in comp['parameters']:
                                if param.get('type') == 'text' and param.get('text'):
                                    nome_contato = at.nome_contato or ""
                ...
                # Extrai a mídia diretamente dos componentes enviados (mesmo se o processamento de texto falhar)
                if components_to_send:
                    header_params_safe = next((c.get('parameters', []) for c in components_to_send if c.get('type') == 'header'), [])
                    for hp in header_params_safe:
                        if 'image' in hp:
                ...
                try:
                    ...
                    if user.wbp_business_account_id:
                        ...
                        if target_template:
                            ...
                            sent_components = components_to_send or []
                            ...
                            for i, param in enumerate(header_params):
                                header_text = header_text.replace(f"{{{{{i+1}}}}}", str(param.get('text', '')))
                            
                            for i, param in enumerate(body_params):
                                body_text = body_text.replace(f"{{{{{i+1}}}}}", str(param.get('text', '')))
                                body_text = body_text.replace(f"{{{{{len(header_params) + i + 1}}}}}", str(param.get('text', '')))

                            # NOVO: Extração dos botões para visualização
