...
from app.agent_manager import start_agent_for_user # <<< NOVO IMPORT
...
async def restart_running_agents():
    """Verifica no DB por agentes que estavam rodando e os reinicia."""
    logger.info("Verificando agentes para reiniciar...")
    db: AsyncSession = SessionLocal()
    try:
        # Usamos BackgroundTasks para não bloquear a inicialização da API
        background_tasks = BackgroundTasks()
        
        stmt = select(models.User).where(models.User.agent_running == True)
        result = await db.execute(stmt)
        users_to_restart = result.scalars().all()

        if not users_to_restart:
            logger.info("Nenhum agente para reiniciar.")
            return

        for user in users_to_restart:
            logger.info(f"Reiniciando agente para o usuário ID: {user.id}")
            start_agent_for_user(user.id, background_tasks) # A função já lida com o background
    finally:
        await db.close()
...
app = FastAPI(
    title="API AtendAI",
    version="1.0.0",
    on_startup=[create_db_and_tables, restart_running_agents],
)
