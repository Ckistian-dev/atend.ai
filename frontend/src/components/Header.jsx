...
const AgentStatus = ({ status, onToggle }) => {
    ...
    const bgColor = isRunning ? 'bg-brand-surface' : 'bg-brand-border-light';
    const textColor = isRunning ? 'text-brand-primary' : 'text-brand-text-muted';
    const text = isRunning ? 'Agente Ativo' : 'Agente Pausado';

    return (
        <button
            onClick={onToggle}
            className={`flex items-center gap-2 text-sm px-3 py-1 rounded-full ${bgColor} ${textColor} transition-transform transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-brand-primary`}
                ...
                <div className="flex items-center gap-4 sm:gap-5">
                    ...
                    {user ? (
                        <button
                            ...
                            className={`flex items-center gap-2.5 text-sm px-3 py-1.5 rounded-full transition-all transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-brand-primary ${
                                user.atendente_online
                                    ? 'bg-brand-surface text-brand-primary hover:bg-brand-surface/80'
                                    : 'bg-brand-border-light text-brand-text-muted hover:bg-brand-surface/80'
                            }`}
                            title={`Você está ${user.atendente_online ? 'Online' : 'Offline'}. Clique para alterar.`}
                        >
                            <span className="relative flex h-2.5 w-2.5">
                                {user.atendente_online && <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-brand-primary opacity-75"></span>}
                                <span className={`relative inline-flex rounded-full h-2.5 w-2.5 ${user.atendente_online ? 'bg-brand-primary' : 'bg-brand-text-muted'}`}></span>
                            </span>
