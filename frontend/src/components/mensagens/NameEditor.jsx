    ...
    return (
        ...
        >
            <form onSubmit={handleSave} className="p-3 space-y-2">
                ...
                <input ref={inputRef} type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Nome do contato" className="w-full px-3 py-2 text-sm rounded-md border-gray-300 shadow-sm focus:ring-brand-primary focus:border-brand-primary" />
                <button type="submit" className="w-full px-3 py-1.5 bg-brand-primary text-white text-sm font-semibold rounded-md hover:bg-brand-primary-active transition-colors flex items-center justify-center gap-2">
                    <Save size={14} /> Salvar
                </button>
            </form>
        </div>
