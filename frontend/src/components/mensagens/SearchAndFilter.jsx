...
const SearchAndFilter = ({ searchTerm, setSearchTerm, activeButtonGroup, toggleFilter, onFilterIconClick, hasActiveFilters }) => {
    ...
    const activeButtonClass = "bg-brand-primary text-white shadow-sm hover:bg-brand-primary-active";
        ...
        <div className="flex-shrink-0 p-3 bg-white border-b border-gray-200 flex flex-col gap-3">
            ...
            <div className="relative w-full">
                ...
                <button
                    ...
                    className="absolute right-1 top-1/2 -translate-y-1/2 p-1.5 rounded-full text-gray-500 hover:bg-gray-200 hover:text-brand-primary"
                    title="Filtrar por status ou tag"
                >
                    <Filter size={18} />
                    {hasActiveFilters && (
                        <span className="absolute top-0 right-0 block h-2 w-2 rounded-full bg-brand-primary ring-2 ring-white" />
                    )}
                </button>
            </div>
