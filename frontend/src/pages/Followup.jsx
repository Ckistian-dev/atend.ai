                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <button
                                onClick={handleSave}
                                disabled={isSaving}
                                className="flex items-center gap-2 px-6 py-2.5 bg-brand-primary text-white rounded-lg font-semibold hover:bg-brand-primary-dark transition-colors disabled:bg-gray-400"
                            >
                ...
                <div className={`space-y-8 transition-opacity duration-500 ${!isActive && 'opacity-50 pointer-events-none'}`}>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {weekDays.map(day => (
                                        <button
                                            ...
                                            className={`px-3 py-1.5 text-xs font-medium rounded-full transition-colors ${config.business_hours.days.includes(day.id) ? 'bg-brand-primary text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}
                                        >
                                            {day.label}
                                        </button>
                                    ))}
                    ...
                    <div className="bg-white p-6 rounded-xl shadow-lg border h-full flex flex-col">
                        <div className="flex justify-between items-center mb-2">
                            ...
                            <button onClick={addInterval} className="flex items-center gap-1.5 px-3 py-1.5 bg-brand-primary text-white rounded-md text-xs font-medium hover:bg-brand-primary-dark">
                            ...
                            {config.intervals.map((interval, index) => (
                                <div key={index} className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg border border-gray-200">
                                    ...
                                    <input
                                        ...
                                        className="w-20 p-1.5 border border-gray-300 rounded-md text-center font-medium text-sm focus:ring-brand-primary focus:border-brand-primary"
                                    />
                                    <select
                                        value={interval.unit}
                                        onChange={e => handleIntervalChange(index, 'unit', e.target.value)}
                                        className="p-1.5 border border-gray-300 rounded-md text-sm bg-white focus:ring-brand-primary focus:border-brand-primary"
                                    >
                    ...
                    <div className="bg-white p-6 rounded-xl shadow-lg border h-full">
                        <h3 className="text-lg font-bold text-gray-800 mb-2 flex items-center gap-2">
                            <CheckCircle size={20} className="text-brand-primary-hover" /> Auto-conclusão
                        </h3>
